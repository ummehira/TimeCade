...
// Enrich request_data with human-readable names by looking up IDs
async function enrichData(data) {
  ...
  try {
    ...
    if (d.time_slot && !d.slot_label) {
      d.slot_label = SLOT_LABELS[d.time_slot] || `Slot ${d.time_slot}`;
    }
    // For reschedule — also look up the old entry details if we have timetable_entry_id
    if (d.timetable_entry_id && (!d.old_subject_name || !d.old_day)) {
      try {
        const old = await pool.query(
          `SELECT t.day, t.slot_label, t.time_slot,
        ...
        if (old.rows.length) {
          ...
          d.old_slot_label   = d.old_slot_label   || o.slot_label;
    ...
    // For delete — look up the timetable entry if we only have timetable_id
    if (d.timetable_id && !d.subject_name) {
      try {
        const e = await pool.query(
          `SELECT t.day, t.slot_label,
                  b.batch_name, s.name AS subject_name,
                  te.full_name AS teacher_name, r.room_id AS room_code
           FROM timetable t
           JOIN batches b  ON t.batch_id   = b.id
...
const getAllRequests = async (req, res) => {
  try {
    ...
    // Enrich all rows with names
    const enriched = await Promise.all(
      r.rows.map(async row => ({
        ...row,
        request_data: await enrichData(row.request_data)
      }))
    );
      ...
      if (request.request_type === 'create') {
        await client.query(
          ...
          [data.batch_id, data.subject_id, data.teacher_id, data.room_id,
           data.day, data.time_slot, SLOT_LABELS[data.time_slot], data.is_lab || false]
        );
        ...
        await client.query(
          ...
          [data.batch_id, data.subject_id, data.teacher_id, data.room_id,
           data.day, data.time_slot, SLOT_LABELS[data.time_slot], data.is_lab || false, entryId]
        );
...
const rejectRequest = async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `UPDATE admin_requests SET status='rejected',reviewed_by=$1,review_note=$2,reviewed_at=NOW()
       WHERE id=$3 AND status='pending' RETURNING *`,
      [req.user.id, review_note, id]
    );
    if (!r.rows.length) return res.status(404).json({ message: 'Request not found or already reviewed.' });
