    ...
    <div className="page-content">
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'14px',marginBottom:'20px' }}>
        {[
          ...
          { label:'My Batch',          value:batchName,           color:'#1e5a6e', icon:Users    },
      ...
      <div style={{ display:'grid',gridTemplateColumns:'260px 1fr',gap:'16px',marginBottom:'16px' }}>
        ...
        <div className="card">
          <div className="card-header">
            <h2 style={{ display:'flex',alignItems:'center',gap:'8px' }}><Clock size={15}/> Today &mdash; {today}</h2>
          </div>
                ...
                <tbody>
                  {todayClasses.map(e=>(
                    <tr key={e.id} style={{ borderBottom:'0.5px solid #e8edf0' }}>
                      ...
                      <td style={{ padding:'11px 14px' }}><span style={{ background:'#f0f4f7',color:'#2d4a5a',border:'1px solid #c8d8e0',borderRadius:'6px',padding:'2px 9px',fontSize:'11px',fontWeight:'600' }}>{e.room_code||e.room_id}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody>
                  {[...new Map(entries.map(e=>[e.subject_name,e])).values()].map(e=>(
                    <tr key={e.id} style={{ borderBottom:'0.5px solid #e8edf0' }}>
                      ...
                      <td style={{ padding:'9px 14px' }}><span style={{ background:'#f0f4f7',color:'#2d4a5a',border:'1px solid #c8d8e0',borderRadius:'6px',padding:'2px 8px',fontSize:'11px',fontWeight:'600' }}>{e.room_code||e.room_id}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="page-content">
              ...
              <tbody>
                {DAYS.map(day=>{
                  ...
                  SLOTS.forEach(slot=>{
                    ...
                    const cell = dayEntries.find(e=>parseInt(e.time_slot)===slot.id);
                    if(cell && cell.is_lab){ cells.push({ slot, cell, span:3 }); skip=2; }
                    else { cells.push({ slot, cell: cell||null, span:1 }); }
                  });
                  return(
                    <tr key={day} style={{ background:isToday?'#f0f7ff':'transparent' }}>
                      ...
                      {cells.map(({ slot, cell, span })=>(
                        <td key={slot.id} colSpan={span} style={{ border:'1px solid #e8edf0',padding:'4px',verticalAlign:'top',height:'78px',background:isToday?'#f8fbff':'white' }}>
                          {cell&&(
                            <div style={{ background:'#2d4a5a',borderRadius:'7px',padding:'7px 9px',color:'white',fontSize:'10px',lineHeight:'1.4',height:'100%',boxSizing:'border-box' }}>
                              ...
                              <div style={{ opacity:0.7,fontSize:'9px',marginTop:'2px',display:'flex',gap:'6px' }}>
                                ...
                                {cell.is_lab&&<span style={{ background:'rgba(255,255,255,0.2)',borderRadius:'3px',padding:'0 4px' }}>Lab 3h</span>}
                              </div>
                            </div>
                          )}
                        </td>
      ...
      <div className="card" style={{ marginTop:'18px' }}>
        <div className="card-header">
          <h2 style={{ display:'flex',alignItems:'center',gap:'8px' }}><Clock size={15}/> All Classes &mdash; Detailed List</h2>
        </div>
        <div className="card-body" style={{ padding:0 }}>
          <table style={{ width:'100%',borderCollapse:'collapse',fontSize:'12px' }}>
              ...
              {DAYS.flatMap(day=>
                entries.filter(e=>e.day===day).sort((a,b)=>a.time_slot-b.time_slot).map(e=>(
                  <tr key={e.id} style={{ borderBottom:'0.5px solid #e8edf0',background:e.day===today?'#f8fbff':'white' }}>
                    <td style={{ padding:'10px 14px',fontWeight:e.day===today?'700':'400',color:e.day===today?'#2d4a5a':'#5a7080' }}>
                      {e.day}
                      {e.day===today&&<span style={{ marginLeft:'6px',background:'#dbeafe',color:'#1e40af',fontSize:'9px',fontWeight:'700',borderRadius:'4px',padding:'1px 5px' }}>TODAY</span>}
                    </td>
                    ...
                    <td style={{ padding:'10px 14px' }}>
                      {e.is_lab
                        ?<span style={{ background:'#fef3c7',color:'#d97706',border:'1px solid #fde68a',borderRadius:'6px',padding:'2px 8px',fontSize:'10px',fontWeight:'700' }}>Lab</span>
                        :<span style={{ color:'#c8d8e0',fontSize:'11px' }}>-</span>
                      }
                    </td>
                  </tr>
                ))
                  ...
                  <div key={e.subject_id} style={{ background:'white',border:'0.5px solid #dde3e8',borderRadius:'12px',overflow:'hidden',borderTop:'3px solid #2d4a5a' }}>
                    <div style={{ padding:'16px 18px' }}>
                      ...
                      <div style={{ fontSize:'12px',color:'#5a7080',marginBottom:'3px',display:'flex',alignItems:'center',gap:'6px' }}>
                        ...
                        {e.teacher_name||'No supervisor assigned'}
                      </div>
